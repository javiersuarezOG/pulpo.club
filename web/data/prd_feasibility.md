...
_Generated: 2026-09-19T08:32:31.072317+00:00_  
_Catalog size: **68 listings**_  
...
| `url` | 68 | 100.0% |
| `title` | 68 | 100.0% |
| `description>20` | 68 | 100.0% |
| `price_usd` | 68 | 100.0% |
| `first_seen_at` | 68 | 100.0% |
| `scraped_at` | 68 | 100.0% |
| `photo_urls>0` | 68 | 100.0% |
| `photos_count>0` | 68 | 100.0% |
| `property_type!=land` | 68 | 100.0% |
| `days_listed` | 68 | 100.0% |
| `broker_name` | 66 | 97.1% |
| `lat` | 62 | 91.2% |
| `lng` | 62 | 91.2% |
| `is_in_development` | 23 | 33.8% |
| `department` | 21 | 30.9% |
| `zone` | 20 | 29.4% |
| `is_beachfront` | 14 | 20.6% |
| `is_repriced` | 3 | 4.4% |
| `area_m2` | 0 | 0.0% |
| `price_per_m2` | 0 | 0.0% |
| `zone_specific` | 0 | 0.0% |
...
| `has_ocean_view` | 8 | 11.8% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `has_paved_access` | 6 | 8.8% | ≥ 40% | 🟡 computed only, below UI gate |
| `has_power` | 4 | 5.9% | ≥ 40% | 🟡 computed only, below UI gate |
| `has_water` | 21 | 30.9% | ≥ 40% | 🟡 above 15% gate, below PRD target |
| `has_water_body` | 11 | 16.2% | ≥ 15% (gate) | 🟢 surface-eligible |
| `is_agricultural` | 7 | 10.3% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `is_beachfront` | 14 | 20.6% | ≥ 15% | 🟢 meets PRD target |
| `is_commercial` | 5 | 7.4% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `is_flat` | 13 | 19.1% | ≥ 15% (gate) | 🟢 surface-eligible |
| `is_motivated` | 7 | 10.3% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `is_on_beach` | 14 | 20.6% | ≥ 15% (gate) | 🟢 surface-eligible |
| `is_on_lake` | 0 | 0.0% | ≥ 15% (gate) | 🔴 below 5% — needs scraper depth |
| `is_tourist` | 20 | 29.4% | ≥ 15% (gate) | 🟢 surface-eligible |
| `is_walk_to_beach` | 5 | 7.4% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `has_sewage` | 0 | 0.0% | ≥ 15% (gate) | 🔴 below 5% — needs scraper depth |
| `is_repriced_text` | 0 | 0.0% | ≥ 15% (gate) | 🔴 below 5% — needs scraper depth |
| `zoning_residential` | 19 | 27.9% | ≥ 15% (gate) | 🟢 surface-eligible |
| `zoning_tourist` | 0 | 0.0% | ≥ 15% (gate) | 🔴 below 5% — needs scraper depth |
| `land_commercial` | 6 | 8.8% | ≥ 15% (gate) | 🟡 computed only, below UI gate |
| `land_recreational` | 1 | 1.5% | ≥ 15% (gate) | 🔴 below 5% — needs scraper depth |
...
| Definition | Hits | % |
|---|---:|---:|
| ANY 1 of 3 utility signals (relaxed) | 25 | 36.8% |
| ALL 3 of 3 utility signals (PRD spec) | 2 | 2.9% |
