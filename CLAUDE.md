...
- **Visual fidelity:** Home/Discover/Browse/Detail are diffed against `docs/design-references/` in every PR that touches them. Visual deviation needs a one-line justification. Active references are indexed in [docs/design-references/README.md](docs/design-references/README.md) — HeroV5 desktop + mobile, browse mobile fallback-density. Capture refresh comes from **production** (anonymous), not Vercel preview (SSO-gated, Pro-authed).
