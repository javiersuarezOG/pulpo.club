...
- [x] **Smoke-test env pollution (`PULPO_SOURCES` leak):** resolved by `conftest.py::_hermetic_offline_env` (autouse, session-scoped) which strips `PULPO_SOURCES` from the env under `PULPO_OFFLINE=1`. The original failure mode involved a now-defunct `kazu` slug in a local `.env`; the slug was retired during Phase C cleanup and the stale `web/data/scraper_failures/kazu_*.json` snapshots were deleted in the same PR.
...
- [ ] **Real Stripe end-to-end (Checkout + Customer Portal)** with a Stripe test card (4242 4242 4242 4242, any future date, any CVC). Steps:
  ...
  2. Click "Upgrade — $4.99/month" on the Plans page → redirect to `https://checkout.stripe.com/...` lands.
