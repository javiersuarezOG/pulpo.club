...
# ─── encuentra24 t_or_fh_m → t_full (ships in the real photo_config.json) ──
#
# These exercise the SHIPPED config, not an in-memory stub, so a regression
# in photo_config.json (strategy reverted to noop, rule typo) fails here.
# encuentra24 sets validate_via_head: true; we neutralise the HEAD network
# call so the test stays pure/offline and asserts the string transform only.


def _no_head_validation(monkeypatch):
    """Make _head_validated a passthrough so url_replace is tested purely."""
    helper.load_photo_config.cache_clear()
    monkeypatch.setattr(helper, "_head_validated", lambda new, original: new)


def test_encuentra24_medium_upgraded_to_full(monkeypatch):
    _no_head_validation(monkeypatch)
    out = helper.upgrade_photo_urls(
        "encuentra24",
        ["https://img.encuentra24.com/x/t_or_fh_m/v1/foo.jpg"],
    )
    assert out == ["https://img.encuentra24.com/x/t_full/v1/foo.jpg"]


def test_encuentra24_already_full_is_idempotent(monkeypatch):
    _no_head_validation(monkeypatch)
    already = "https://img.encuentra24.com/x/t_full/v1/foo.jpg"
    assert helper.upgrade_photo_urls("encuentra24", [already]) == [already]


def test_non_encuentra24_source_unchanged_by_e24_rule(monkeypatch):
    _no_head_validation(monkeypatch)
    # remax ships strategy: noop — a t_or_fh_m URL (hypothetical) must NOT
    # be rewritten by encuentra24's rule.
    urls = ["https://media.remax.test/x/t_or_fh_m/v1/foo.jpg"]
    assert helper.upgrade_photo_urls("remax", urls) == urls
