...
def test_consecutive_red_returns_ok_when_no_history(tmp_path):
    ok, msg = check_source_consecutive_red(tmp_path)
    assert ok
...
def test_consecutive_red_alerts_on_two_red_in_a_row(tmp_path):
    ...
    _write_health([
        _row("remax", "2026-05-03T12:00:00+00:00", "red",  count=0,
             error_class="ParseError"),
        _row("remax", "2026-05-04T12:00:00+00:00", "red",  count=0,
             error_class="ParseError"),
        _row("goodlife", "2026-05-04T12:00:00+00:00"),
    ], tmp_path)
    ok, msg = check_source_consecutive_red(tmp_path)
    assert not ok
    assert "remax" in msg
...
def test_consecutive_red_quiet_after_recovery(tmp_path):
    ...
    _write_health([
        _row("remax", "2026-05-03T12:00:00+00:00", "red", count=0),
        _row("remax", "2026-05-04T12:00:00+00:00", "green"),
    ], tmp_path)
    ok, _msg = check_source_consecutive_red(tmp_path)
    assert ok
...
def test_consecutive_red_quiet_with_only_one_red_run(tmp_path):
    _write_health([
        _row("remax", "2026-05-04T12:00:00+00:00", "red", count=0,
             error_class="NetworkTimeout"),
    ], tmp_path)
    ok, _ = check_source_consecutive_red(tmp_path)
...
def test_consecutive_red_handles_two_sources_simultaneously(tmp_path):
    rows = [
        ...
        _row("remax", "2026-05-04T12:00:00+00:00", "red", count=0,
             error_class="ParseError"),
        _row("century21", "2026-05-03T12:00:00+00:00", "red", count=0,
             error_class="HTTPError"),
        _row("century21", "2026-05-04T12:00:00+00:00", "red", count=0,
             error_class="HTTPError"),
    ]
...
def test_consecutive_red_skips_malformed_lines(tmp_path):
    ...
    path.write_text(
        "{not json\n"
        + json.dumps(_row("remax", "2026-05-03T12:00:00+00:00", "red", count=0)) + "\n"
        + json.dumps(_row("remax", "2026-05-04T12:00:00+00:00", "red", count=0)) + "\n"
    )
    ok, msg = check_source_consecutive_red(tmp_path)
    assert not ok
    assert "remax" in msg
