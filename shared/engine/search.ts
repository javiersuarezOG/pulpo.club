// shared/engine/search.ts — keyword matching over the catalog.
//
// Moved verbatim from web/app/lib/search-match.ts. applyFilters calls
// tokenize/matchesQuery on every row, so the API and the bot need the
// same matcher the website uses or an identical query returns different
// results per channel.
//
// The original header below explains the design (exact-substring, not
// fuzzy) and is worth keeping.
//
