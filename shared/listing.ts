// shared/listing.ts — the Listing shape, shared by every channel.
//
// Moved verbatim from web/app/data/types.ts. It sits in shared/ because
// it is the cross-channel contract, not a web detail: the same shape is
// what /api/v1/listings serves, what the MCP tools return, and what the
// Telegram bot renders. web/app/data/types.ts re-exports it, so no
// existing import path in the web app changed.
//
// Types only — no runtime values — so this move cannot affect
// behaviour, only what the compiler checks.
//
