...
### Access-option independence (hard architecture constraint — Javi, 2026-06-12)
The three access paths — **① Join free (email)**, **② Go Pro (Stripe)**, **③ Sign in (Clerk)** — must each be **independently toggleable / reorderable later** (to A/B or optimize the funnel) **without breaking the email flows or the post-signup/signin user flows**. Build it this way from the start:

1. **Config-driven access registry** — one source of truth (mirror the existing `web/app/config/registry.ts` pattern), e.g. `ACCESS_OPTIONS = [{ id:"free_email", enabled, order, surfaces:[...] }, { id:"go_pro", … }, { id:"sign_in", … }]`. The shared `AccessBlock` renders whatever's enabled, in order. Toggling/reordering = a flag/config change, **no component surgery**. Each surface (hero, save-gate, listing-gate, plans) selects its subset from the registry.
2. **Decoupled actions** — each option owns a self-contained handler: free → `subscribeEmail` + `becomeFreeMember` (Resend); pro → `startCheckout` (Stripe); sign-in → Clerk `openSignIn`. No handler calls another; disabling one never references the others. The three backends are already separate endpoints — keep them so.
3. **Downstream keys on OUTCOME, not entry** — the welcome email, member state, top-3 unlock, and Pro state all key off the **resulting state** (`isFreeMember` / `isPaid`), never off which button was pressed. So removing "Sign in" can't break the free→member flow; removing "Go Pro" from a modal can't break the email welcome; etc.
4. **Free→Pro linking degrades gracefully** — threading the captured email into checkout (PR-C) is best-effort: if the email path is disabled on a surface, checkout still works (Stripe collects the email); if Stripe is disabled, the email path is untouched.
5. **Per-surface contract test** — a test asserts each surface renders exactly its configured, enabled options in order, and that toggling one option's flag changes only that option (no cross-effect).
